...
export default function Sidebar({ isOpen, setIsOpen, isMobileView, isTabletView }) {
  const menuItems = [
    {
  ...
  return (
    <>
      ...
      {isMobileOrTablet && (
        <div
          className={`
            fixed inset-y-0 left-0 w-64 md:w-72 bg-[#F3F4F4] shadow-lg z-50
        ...
        >
            ...
            <div className="px-3 md:px-4">
              <button
                className={`
                  ...
                  hover:bg-[#3A78DE] transition w-full font-inter font-medium text-[14px] md:text-[15px]
                  leading-[20px] tracking-[-0.6%] text-center
                `}
              >
                <span className="text-[14px] md:text-[15px] font-medium">
                  Review Documents
                </span>
                <Plus size={isTabletView ? 20 : 18} />
              </button>

              <nav className="space-y-1 md:space-y-2 mt-4">
                {menuItems.map((item) => (
                  <NavLink
                    key={item.label}
                    to={item.path}
                    onClick={() => setIsOpen(false)}
                    className={({ isActive }) =>
                      `flex items-center gap-3 p-3 md:p-4 rounded-lg transition-all duration-200
                      ${
                        isActive
          ...
          {/* Footer */}
          <div className="flex-shrink-0 p-4 md:p-5 border-t bg-gray-100/50">
            <div className="text-xs md:text-sm text-gray-500 text-center">Version 1.0.0</div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <nav className="flex-1 overflow-y-auto mt-6 px-2">
            <button
              className={`
                flex items-center gap-2 py-3 px-4 rounded-lg bg-[#4A88EE] text-[#F9FAFA]
                hover:bg-[#3A78DE] justify-between transition w-full font-inter font-medium 
                text-[14px] leading-[20px] tracking-[-0.6%] text-center
...
function SidebarItem({ icon, label, path, isOpen }) {
  return (
    ...
    >
      ...
      />
      {isOpen && (
        <span className="truncate font-[500] align-middle text-[14px]">
          {label}
        </span>
      )}
    </NavLink>
